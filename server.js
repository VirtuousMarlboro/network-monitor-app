...
async function autoPingAllHosts() {
    ...
    for (const hostData of monitoredHosts) {
        ...
        // ALSO: Initialize tracking for hosts that are already offline but not yet tracked
        // (e.g., after server restart, or first check finds host already down)
        if (newStatus === 'offline') {
            // If host is offline but we don't have a tracking timestamp, start tracking now
